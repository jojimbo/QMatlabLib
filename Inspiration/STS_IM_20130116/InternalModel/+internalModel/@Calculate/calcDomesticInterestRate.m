function [obj, instrument] = calcDomesticInterestRate(obj, instrument)
%% calcInterestRate
% |obj = calcDomesticInterestRate(obj, instrument)|
%
...
switch instclass
    ...
    case 'internalModel.Instruments.FXIRSwap'
        targets = instrument.DomesticPaymentTimes;
...
curve = obj.scenCol.Curves.findCurveByName(instrument.DomesticCurve);
...
for iTenor = 1:numel(targets)
    ...
    if ~isempty(obj.shockedInterestLib)
        idxInLib = strcmp(obj.shockedInterestLib(:, 1), instrument.DomesticCurve) ...
    ...
    if any(idxInLib)
        ...
        idxPca = strcmp(instrument.Currency, {obj.scenCol.PcaValues.currency});
        
        if ~any(idxPca)
            error('Calculate:IdxNotFound', ['Currency ' obj.Currency ...
                ' not found']);
        end
        ...
        for iPca = 1:nrPrincipalComponents
            riskFactor  = strcat('IR_', instrument.Currency, 'PC', num2str(iPca));
        ...
        obj.shockedInterestLib{end + 1, 1} = instrument.DomesticCurve;
...
switch instclass
    case 'internalModel.Instruments.ZeroCouponBond'
        instrument.interestRate = rTot;
    case 'internalModel.Instruments.FXForward'
        instrument.domesticInterestRate = rTot;
    case 'internalModel.Instruments.FXSwap'
        instrument.domesticInterestRate = rTot;
    case 'internalModel.Instruments.FXIRSwap'
        instrument.domesticInterestRate = rTot;
