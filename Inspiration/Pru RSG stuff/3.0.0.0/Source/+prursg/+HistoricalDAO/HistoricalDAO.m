classdef HistoricalDAO
    ...
    methods
        function obj = HistoricalDAO(folderPath)
        ...
        function writeData(obj, fileName, dataSeries)
            import prursg.Engine.*;
            fid = fopen(fullfile(prursg.Util.ConfigurationUtil.GetRootFolderPath(), obj.folderPath, [fileName '.csv']),'w');
            if fid < 0
        ...
        function dataObj = createDataObj(obj,rawNumbers, rawText, numDim, dateStart, dateEnd)
            ...
            datesAll = cell(size(rawText,2)-numDim,1);
            for i = numDim+1:size(rawText,2)
                datesAll{i-numDim} = rawText{1,i};
            end
        ...
        function [rawNumbers rawText] = getRawData(obj,dataName)
            ...
            fid = fopen(fullfile(prursg.Util.ConfigurationUtil.GetRootFolderPath(), obj.folderPath, [dataName '.csv']));
            rawData = fgetl(fid);
            ...
            while ischar(rawData)
                [someText someNumbers] = obj.convertDataString(rawData);
                rawText{lineCount} = someText;
                
                if (lineCount > cellSize)
                    rawText = [rawText;cell(1,1000)];
        ...
        function [rawText rawNumbers] = convertDataString(obj, s)
            ...
            if (isempty(Pos))
                errorCause = MException('RSG_HistoricalDAO:invalidCharactersInFile', 'Error trying to parse a line in the file, the input file may be invalid.');
                throw(errorCause);
            else
                rawText = s(1:Pos(1)-1);
                rawNumbers = [];
