%% Test_CalibrationData
%
% SUMMARY:
%       Regression test for calibration to data
%%

function test_suite = Test_CalibrationData() %#ok<STOUT,FNDEF>
    % The first test must return an instance of initTestSuite
    disp('Initialising Test_CalibrationData')
    initTestSuite;
    disp('Executing Test_CalibrationData')
end
...
function Test_CalibrationData1()	%#ok<DEFNU>
    ...
    % Clear down the DB (or not) and run calibration to data
    TestUtil.RebuildScenarioDB();
	[UserMsg ScenSetID] = RSGSimulate('Test40.xml');
    [UserMsg XMLFilePathOut] = RSGCalibrate('Test39.xml');
