classdef ValidationRulesSet4 < prursg.Engine.ValidationRules
    ...
    methods
        function obj = ValidationRulesSet4()
...
function [measureNew, valueNew] = validationSet(simData)
    ...
    measureNew = num2cell([0.05:0.05:0.9,0.91:0.01:0.99,0.9902:0.0002:0.999,0.9991:0.0001:0.9999,0.99991:0.00001:1]);
    valueNew = num2cell(prctile(simData,[5:5:90,91:1:99,99.02:0.02:99.9,99.91:0.01:99.99,99.991:0.001:100],1));
    valueNew = valueNew';    
...
function resultFileName = Process(obj, batchIndex, risks, scenarioSet, stoValues, reportPath)
    ...
    rowSize = nSubRisks;
    colSize = length(detScenarios) + 91;  % Mean, Median, Std dev, 4 percentiles
    ...
    for i1=1:nRisks
        ...
        for i2 = 1:length(expandedNames) 
            ...
            if currentRow == 1 
                valData(currentRow, currentCol + 3:currentCol + 93) = measureNew;  %row 1 contains the names of the stats
            end            
            valData(currentRow + 2, currentCol + 3:currentCol + 93) = valueNew;            
        end
    end
    ...
    rowSize1 = (numDetScen * nSubRisks) + (nSubRisks * 91);
