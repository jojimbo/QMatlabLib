...
function [itemNew, measureNew, valueNew] = CorrelSet(riskName1,expandedNames1,simData1,riskName2,expandedNames2,simData2)
            ...
            measureNew = [];
            valueNew = [];
            simDataAdj1 = simData1 - mean(simData1);
            simDataAdj2 = simData2 - mean(simData2);
            
            itemNew = [itemNew ; {[riskName1 expandedNames1]}]; 
            measureNew = [measureNew ; {[riskName2 expandedNames2]}];
            valueNew = [valueNew ; {sum(simDataAdj1.*simDataAdj2)./(sqrt(sum((simDataAdj1).^2).*sum((simDataAdj2).^2)))}];
...
function resultFileName = Process(obj, batchIndex, risks, scenarioSet, stoValues, reportPath)
    ...
    simDataRanksArray = zeros(length(stoValues{1}),nRisks);
    ...
    for i1 = 1:nRisks
        ...
        simDataRanks = tiedrank(simData);
        simDataRanksArray(:,i1) = simDataRanks;
    end
    
    
    NoSims = length(simData);
    
    %SpearmanScaleFactor = 6/((NoSims*(NoSims.^2-1)));
   
    
    currentCol2 = 0;
    for i1=1:nRisks
        expandedNames = baseScenario.expandedUniverse(risks(i1).name).getExpandedNames;
        simDataRank1 = simDataRanksArray(:,i1);
        ...
        for i3 = i1:nRisks
            ...
            simDataRank2 = simDataRanksArray(:,i3);
            i4 = length(expandedNames2); 
 
            [itemNew, measureNew, valueNew] = CorrelSet(risks(i1).name,expandedNames{i2},simDataRank1,risks(i3).name,expandedNames2{i4},simDataRank2);
