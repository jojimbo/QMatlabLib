...
%
% *The main purpose of this class is to takes a Volatility Surface and 
% extrapolates the forward volatilities of the surface using an exponential 
% smoothing method to a long term target*
%
...
%
% The rest of the methods are the breakding down steps of
% |[CreateExtrapolatedSurface()]|.
% Each method can be called separately.
% 
...
%
% *|7) [CreateSpotVolSurfaceBiLinearInterpolation()]|* - Function bilneraly 
% interpolates a spot volatility surface using the total variance method
%
...
% than the previous row value then it will be replaced by the previous row value
% 
% *|9) [CreateContStatsFromArithmeticStats]|* - Uses lognormal approximation 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
 ...
 function obj = bsEquityForwardVolExtrapolation (...
   EquityReturnEstimate,EquityReturnVol, ...
   EquityEstimateType,EquityFwdVol_MeanReversionSpeed,LastLiquidMaturity,minLiquidStrike,... 
   ...
   = obj.CreateLogRtnStatsFromEstimateStats(EquityReturnEstimate,...
   EquityReturnVol,EquityEstimateType);
...
% _Data Type_: 1-dim array
%
%|[VolatilityType]| - This is volatility type of the output 
%
% _Data Type_: String value
%
...
%
% STEP 2: Split Option Maturity Profile in two using the function |[SplitMaturityOutputProfile()]|. 
...
%
% STEP 9: Finally, we output the user specified spot volatility type. The default 
% type is equity IV which is calculate using another call to the bootstrap method 
% |[bsEquityIV_1FHW_StochRateEffect]| to create a spot implied volatility surface
% |[CreateExtrapolatedSurfaceReturn]| by adding back the stochastic interest 
% rate effect to the |[CombinedSpotVolSurface]| 
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%       
   function CreateExtrapolatedSurfaceReturn = CreateExtrapolatedSurface...
        ...
        OutputMaturityProfile, OutputStrikes, VolatilityType, Truncate)
     ...
     switch lower(VolatilityType)
         
         case 'stddev'
            CreateExtrapolatedSurfaceReturn = CombinedSpotVolSurface;
         otherwise
...
% _Data Type_: single value
%
% |[MeanReversionLevel]| - $\sigma_{LT}$ - Target value in the long term
%
% _Data Type_: single value
%
...
% with maturity $T_i$ and moneyness $M_j$, then
%
% $y_{ij} = e^ {(- k \times T_i)} \times x_{ij} + (1- e^{(- k \times T_i)}) \times \sigma_{LT}$
%
...
% the ones longer than the last liquid maturity being truncated; 
% |[ReducedStrikeMoneyness]| is a 1-dim array of strike moneyness with the ones
% outside the liquid strikes being truncated;
  ...
  function [ReducedVolatilitySurface,ReducedOptionMaturities, ...
      ...
      if isempty(llpstrike_maturity_index)
        ...
        % llpstrike_maturity_index = max(llpstrike_maturity_index(1,1)-1,0);
         llpstrike_maturity_index = size(OptionMaturities, 2) - length(llpstrike_maturity_index);
      end    
      ...
      if isempty(llpstrike_min_index) 
        ...
        % llpstrike_min_index = min(llpstrike_min_index +1, ...
         ...
         llpstrike_min_index =length(llpstrike_min_index);         
      end
      ...
      if isempty(llpstrike_max_index) 
       ...
       %  llpstrike_max_index = max(llpstrike_max_index-1, 0);
          llpstrike_max_index = size(StrikeMoneyness, 2)  - length(llpstrike_max_index);
      end
...
% *_Calculation_*
%
% Locate the last liquid maturity. Truncate the values beyond the last liquid point
...
% remaining maturities, i.e., those being truncated from the |[OutputProfile]|. 
...
% *_Inputs_*
%
% |[SpotVolatilitySurface]| - A 2-dim array of spot volatility surface 
...
% negative number in the next calculation
%
...
% *_Inputs_*
%
% |[FwdVolatilitySurface]| - A 2-dim array of forward volatility surface 
% with "maturities" against "moneyness"
% 
% _Data Type_: 2-dim array
% 
...
% *_Description_*  
%
% Function bilneraly interpolates a spot volatility surface using the total variance method 
%
% *_Inputs_*
%
...
% 
% This is to prevent negative values in the later calculation of forward volatilities
%
...
% _________________________________________________________________________________________
%
%% |9) [CreateLogRtnStatsFromEstimateStats()]|
%
% '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
%
% *_Description_*  
...
% *_Inputs_*
%
% |[ReturnEstimate]| - $\mu_{Est}$, the mean or median return estimate in the long
% term
% 
% _Data Type_: single value
% 
% |[ReturnVol]| - $\sigma_{Est}$, the volatility estimate in the long
% term
%
% _Data Type_: single value
% 
% |[ReturnType]| - The value is either 'Mean' or 'Median' which indicates 
% the estimate statistic type of the ReturnEstimate
%
% _Data Type_: string
%
% *_Outputs_*
%
% |[ContinuousMean,ContinuousVol]| - an arrary containing two single values, one is
...
% *_Calculations_*
%
% If ReturnType = 'Mean' then
%
% $\sigma_{LT} = \sqrt {log \Big( ( \frac{\sigma_{Est}}{1+\mu_{Est}} )^2
% +1 \Big)}$
%
% $\mu_{LT} = log (1+ \mu_{Est}) - \frac{\sigma_{LT}^2}{2}$
%
% If ReturnType = 'Median' then
%
% $\sigma_{LT} = \sqrt { \log ( \sqrt {1 + 4 (\frac{\sigma_{Est}}{1+\mu_{Est}}
% )^2  }) ) - \log(2)  }$
%
% $\mu_{LT} = log (1+ \mu_{Est})$
%        
%   
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function [ContinuousMean,ContinuousVol] = ...
      CreateLogRtnStatsFromEstimateStats(obj,ReturnEstimate,ReturnVol, ReturnType)
    
     switch lower(ReturnType)
      
         case 'mean'
        ContinuousVol = sqrt(log((ReturnVol/(1+ ReturnEstimate))^2 +1));
        ContinuousMean = log(1+ ReturnEstimate) - ContinuousVol^2 /2;
         case 'median'
         ContinuousVol =   sqrt(log(1+ sqrt(1 + 4* ( ReturnVol/(1+ ReturnEstimate) )^2) ) -log(2));
         ContinuousMean = log(1+ ReturnEstimate);  
     end
