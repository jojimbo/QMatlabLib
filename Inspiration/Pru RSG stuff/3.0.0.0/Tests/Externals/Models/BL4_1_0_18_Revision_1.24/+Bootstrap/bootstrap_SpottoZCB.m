...
% *Input Data Series*
%
% *|[InputSpotRates]|* - $S(t,T)$, spot rates at time, $t$, paying at 
% maturity, $t=T$.
% 
...
% coupon bond (ZCB) prices, $B(t,T)$, with a given output frequency profile. 
% The ZCB prices are derived from an input of spot rates, $S(t,T)$, with 
...
% *_Description_*
%
% Function returns a data series of zero coupon bond (ZCB) prices, $B(t,T)$, 
...
% where
%
% $S(t,T)$ : Spot rate at time, $t$, paying at 
% maturity, $t=T$.
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % Methods
        ...
        function results = Bootstrap(obj, DataSeriesIn)
            ...
            freq = str2num(obj.InputSpotRates.compoundingfrequency); 
               ...
               results.description = 'derived SpottoZCBmethod';
