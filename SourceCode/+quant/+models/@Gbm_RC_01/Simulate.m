...
function varargout = Simulate(OBJ, NPERIODS, varargin)
    %% Simulate - Perform simulation(s) of the GBM_RC_01 model
    %
    % Simulate SYNTAX:
    ...
    %
    %% Function Simulate for GBM_RC_01 model
...
if nargin < 2
    error(message('+quant.models:GBM_RC_01:Simulate:TooFewInputs'));
end

if nargout > 3
    error(message('+quant.models:GBM_RC_01:Simulate:TooManyOutputs'));
end
...
if useMatlabMethod(method)
    % Create Matlab gbm sde to use for the simulation
    gbm_obj = gbm(OBJ.drift, OBJ.sigma, 'StartState', StartState);
    [Paths, Times, Z_new] = ...
        simulate(gbm_obj, NPERIODS, 'DeltaTime', dt, 'NTRIALS', nTrials, 'Z', Z, 'Antithetic', isAntithetic);
